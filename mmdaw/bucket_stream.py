...
class BucketStream:
    def __init__(self, gamma, compress=True, alpha=0.1, seed=1234, min_size=None):
        """ """
        self.gamma = gamma
    ...
    def mmd(self, split):
        ...
        return (
            XX + YY - XY,
            int(np.sqrt(end.n_XX)),
            int(np.sqrt(start.n_XX)),
        )  # , XX, YY, XY
    ...
    def merge_buckets(self, bucket_list):
        ...
        XX_str = (
            previous.XX_str + current.XX_str + 2 * current.XY_str[-1]
            if self.logging
            else ""
        )
        ...
        XY_str = (
            [p + c for p, c in zip(previous.XY_str, current.XY_str[:-1])]
            if self.logging
            else ""
        )
                ...
                + [
                    Bucket(
                        self.rng.choice(
                            ...
                            size=min(
                                size, len(previous.elements) + len(current.elements)
                            ),
                            replace=False,
                        ),
